        ...
        for y in range(self.N):
            for x in range(self.M):
                ...
                match self.grid[y][x].kind:
                    case 1.0:
                        canvas.create_rectangle((xx, yy), (xx+CELL_WIDTH, yy+CELL_WIDTH), fill="green", outline="green")
                    case 2.0:
                        canvas.create_rectangle((xx, yy), (xx+CELL_WIDTH, yy+CELL_WIDTH), fill="grey", outline="grey")
                    case 3.0:
                        canvas.create_rectangle((xx, yy), (xx+CELL_WIDTH, yy+CELL_WIDTH), fill="red", outline="red")
...
class Animal:
    ...
    def can_mate(self, month, spawn):
        if self.energy >= self.threshold and \
            ...
            if len(n) > 0:
                x1, y1 = choice(n)
                cub = self.reproduce(territory, x1,y1)
                spawn.append(cub)
                return True
        return False
    def is_dead(self):
